...
import { wrapRTCPeerConnection } from '@rtcstats/rtcstats-js/peerconnection'
import {
  wrapEnumerateDevices,
  wrapGetUserMedia,
} from '@rtcstats/rtcstats-js/media'
...
export function LocalTrace() {
  ...
  trace('init', null, {userAgentData: userAgentData, 'rtcStats.js': '0.0.1'})
  trace('getStats', null, 'getStats() is not supported in this browser.')
  trace('create', null, {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  connectButton.addEventListener('call_ended', (e: CallEndedEvent) => {
    ...
    const rtcStats = trace.collectStats();
    let output = 'RTCStatsDump\n'
    output += `{"fileFormat":3}\n`
    for (let record of rtcStats) {
      output += JSON.stringify(record) + '\n'
    }
    const blobUrl = URL.createObjectURL(new Blob([new TextEncoder().encode(output)], {type: 'text/plain'}))
    window.open(blobUrl)
  })
